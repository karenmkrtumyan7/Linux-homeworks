...
int main(int argc, char *argv[]) {
	...
	pthread_mutex_init(&maximum, NULL);
	s = atoi(argv[1]);
	int t = atoi(argv[2]);
	char path[1024];
	strcpy(path, "../T3/");
	...
	pthread_mutex_lock(&maximum);
  	...
  	for (int i = 0; i < t; ++i) {
  		...
  		memcpy(part + 1, nums + a, sizeof(int) * (b - a));
    		pthread_create(&threads[i], NULL, func2, (void*)part);
